...
// Mock the db module first, before importing the subject under test
vi.mock('../lib/db', () => {
  const dbMock = {
    fieldMappings: {
  ...
  // Define tables as a getter to make it writable in tests
  Object.defineProperty(dbMock, 'tables', {
    get: vi.fn().mockReturnValue([{ name: 'fieldMappings' }]),
    configurable: true
  });
...
// Get the mock objects for assertions
...
describe('Field Mapping Service', () => {
  ...
  beforeEach(() => {
    ...
    // Reset mockDb.tables to default for each test
    Object.defineProperty(mockDb, 'tables', {
      get: vi.fn().mockReturnValue([{ name: 'fieldMappings' }]),
      configurable: true
    });
  });
  ...
  describe('parseAmount helper function', () => {
    // Since parseAmount is private, we'll test it through applyMapping
    ...
    it('should handle empty or non-numeric amount values', () => {
      const csvData = [
        { 'Date': '2025-01-01', 'Description': 'Empty Amount', 'Amount': '', 'Account': 'Checking' },
        { 'Date': '2025-01-02', 'Description': 'Text Amount', 'Amount': 'pending', 'Account': 'Checking' }
      ];
      
      // Allow empty amount for this test
      const mapping: FieldMapping = {
        ...createBasicMapping(),
        mappings: {
          ...createBasicMapping().mappings,
          amount: null // Set to null to test fallback behavior
        }
      };
      
      const result = applyMapping(csvData, mapping);
      
      // Both should be skipped due to missing critical fields
      expect(result.skippedRows).toHaveLength(2);
    });
  });

  describe('determineTypeFromString helper function', () => {
  ...
  describe('Database operations', () => {
    beforeEach(() => {
      ...
      vi.clearAllMocks();
      vi.spyOn(console, 'log').mockImplementation(() => {}); // Suppress console output
    });

    it('should save a new mapping to the database', async () => {
      const mapping: FieldMapping = {
        ...createBasicMapping(),
        name: 'Test Mapping',
        sourceIdentifier: 'Test Bank'
      };
      
      const id = await saveMapping(mapping);
      
      expect(mockDb.fieldMappings.put).toHaveBeenCalledWith(expect.objectContaining({
        id,
        name: 'Test Mapping',
        sourceIdentifier: 'Test Bank'
      }));
    ...
    it('should update an existing mapping', async () => {
      ...
      const id = await updateMapping(mapping);
      
      expect(id).toBe('existing-id');
      expect(mockDb.fieldMappings.put).toHaveBeenCalledWith(mapping);
    });

    it('should throw an error when updating a mapping without an ID', async () => {
      const mapping: FieldMapping = {
        ...createBasicMapping(),
        name: 'No ID Mapping'
      };
      
      await expect(updateMapping(mapping)).rejects.toThrow('Cannot update a mapping without an ID');
    });

    it('should get all saved mappings', async () => {
      const mockMappings = [
        {
          id: 'id1',
          name: 'Mapping 1',
      ...
      (mockDb.fieldMappings.toArray as ReturnType<typeof vi.fn>).mockResolvedValueOnce(mockMappings);
    ...
    it('should return an empty array if accessing fieldMappings throws an error', async () => {
      // Mock toArray to throw an error
      const originalToArray = mockDb.fieldMappings.toArray;
      mockDb.fieldMappings.toArray = vi.fn().mockRejectedValueOnce(new Error('Table not found'));
      ...
      mockDb.fieldMappings.toArray = originalToArray;
    });
  });
});